...
class FinalRelativityVisualization(Scene):
    def construct(self):
        # Title sequence
        ...
        # Create light paths using custom curved lines
        paths = VGroup()
        for image in lensed_images:
            # Create curved path from source to image
        ...
        # GPS satellite - using regular colors to avoid LIGHT_GRAY issue
        satellite = VGroup()
        satellite_body = Rectangle(height=0.2, width=0.5, color=WHITE)
        satellite_panels = VGroup(
            Rectangle(height=0.1, width=1.0, color=YELLOW).next_to(satellite_body, LEFT, buff=0),
            Rectangle(height=0.1, width=1.0, color=YELLOW).next_to(satellite_body, RIGHT, buff=0)
        )
